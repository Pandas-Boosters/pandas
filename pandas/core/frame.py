...
class DataFrame(NDFrame, OpsMixin):
    ...
    def dropna(
        ...
        *,
        ...
        print("DEBUG: dropna called")
         
        if axis in (0, "index"):
           cols = self.columns if subset is None else subset
           for col in cols:
            seen_types = set()
            for val in self[col]:
                if val is not None:
            ...
            if len(seen_types) > 1:
                raise TypeError(
                     f"Mixed data types detected in column '{col}': dropna() requires uniform column types."
                )
